                   ...
                   const float* const filter, const int filterWidth)
{
  ...
  extern __shared__ float localFilter[];
  ...
  for (int j = 0; j < filterWidth; j++) {
    int weightY = y - center + j;
    if (weightY < 0) {
      weightY = 0;
    } else if(weightY >= numRows) {
      weightY = numRows - 1;
    }
    for (int i = 0; i < filterWidth; i++) {
      int weightX = x - center + i;
      if (weightX < 0) {
	weightX = 0;
      } else if(weightX >= numCols) {
	weightX = numCols - 1;
      }
      weightedSum += localFilter[weightIdx++] * inputChannel[weightY * numCols + weightX];
    }
  }
  outputChannel[y * numCols + x] = weightedSum;
}
